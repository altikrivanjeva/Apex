      ...
      <main className="flex-1 flex flex-col items-center py-16">
        ...
        <section className="w-full max-w-5xl mx-auto mb-12 px-4 py-10 bg-gradient-to-r from-orange-100 via-blue-50 to-blue-100 rounded-2xl shadow-lg flex flex-col items-center">
          ...
          <p className="text-xl text-blue-700 text-center mb-2" style={fontOpenSans}>
            Suplementet më të mira për performancë, muskuj dhe shëndet. Zgjidh, shto në shportë dhe bli me një klik!
          </p>
        </section>
          ...
          {cart.length > 0 && (
            <div className="bg-white rounded-2xl shadow-lg p-6 border border-blue-100 mb-6">
              ...
              <div className="mt-4 text-right font-bold text-blue-900" style={fontMontserrat}>
                Totali: {cart.reduce((acc, item) => acc + item.quantity * Number(item.price.replace('€', '')), 0).toFixed(2)} €
              </div>
              <button
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-10 w-full max-w-7xl px-4">
          {products.map((product) => (
            <div
              key={product.id}
              className="bg-white rounded-2xl shadow-lg p-6 flex flex-col items-center border border-blue-100 transition-transform duration-200 hover:scale-105 hover:shadow-2xl"
            >
              <img
                src={product.img}
                alt={product.name}
              ...
              <span
                className="px-4 py-1 bg-blue-100 text-blue-700 font-bold rounded mb-4"
                style={fontOpenSans}
              >
                {product.status}
              </span>
              <button
                ...
                className="px-6 py-2 bg-orange-500 text-white font-bold uppercase rounded shadow hover:bg-orange-600 transition"
                style={fontMontserrat}
              >
                Bli produktin
              </button>
        ...
        <div className="mt-16 text-center text-blue-900 text-xl font-bold" style={fontMontserrat}>
          Zgjidh suplementin që të përshtatet dhe fillo rrugëtimin tënd drejt suksesit!
        </div>
      </main>
      <Footer />
    </div>
